...
- **client.ts**: Low-level API fetching with 60s revalidation and browser User-Agent spoofing
- **engine.ts**: Business logic for building live league tables (handles the N+1 query problem with `p-limit` concurrency control)
- **types.ts**: TypeScript types for FPL API responses
...
### Live Scoring Engine
The `buildLiveTable` function in `engine.ts`:
