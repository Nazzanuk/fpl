...
export async function getPlayerDetail(elementId: number, leagueId: number) {
	const [events, teams, elements, elementSummary] = await Promise.all([
		getBootstrapEvents(),
		getBootstrapTeams(),
		getBootstrapElements(),
	...
	if (leagueId && currentEvent) {
		leagueOwnership = await getPlayerLeagueOwnership(
			elementId,
			leagueId,
			currentEvent.id,
		);
	...
	return {
		...
		fixtures:
			elementSummary.fixtures?.slice(0, 5).map((f: any) => ({
				eventId: f.event,
				eventName: `GW${f.event}`,
				opponentId: f.team_h === element.team ? f.team_a : f.team_h,
				opponent:
					f.team_h === element.team
						? teams.find((t: any) => t.id === f.team_a)?.name || "Unknown"
						: teams.find((t: any) => t.id === f.team_h)?.name || "Unknown",
				opponentShort:
					f.team_h === element.team
						? teams.find((t: any) => t.id === f.team_a)?.short_name || "UNK"
						: teams.find((t: any) => t.id === f.team_h)?.short_name || "UNK",
				isHome: f.team_h === element.team,
				difficulty:
					f.team_h === element.team ? f.team_h_difficulty : f.team_a_difficulty,
				kickoffTime: f.kickoff_time,
			})) || [],
		history:
			elementSummary.history?.map((h: any) => ({
				gameweek: h.round,
				opponent:
					teams.find((t: any) => t.id === h.opponent_team)?.short_name || "",
				opponentShort:
					teams.find((t: any) => t.id === h.opponent_team)?.short_name || "",
				isHome: h.was_home,
...
async function getPlayerLeagueOwnership(
	elementId: number,
	leagueId: number,
	eventId: number,
) {
