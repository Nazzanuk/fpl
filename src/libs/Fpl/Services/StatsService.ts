...
import { getLeagueData } from "../Data/Client/FPLApiClient";
...
export async function getLeagueStats(leagueId: number): Promise<LeagueStats> {
	const [elements, leagueData] = await Promise.all([
	...
	const averageScore =
		scores.reduce((sum: number, s: number) => sum + s, 0) / totalManagers;
	const highestScore = Math.max(...scores);
	const lowestScore = Math.min(...scores);

	// Get most captained, transferred in/out from bootstrap (global stats)
	const mostCaptained = elements
	...
	const mostTransferredIn = elements
	...
	const mostTransferredOut = elements
